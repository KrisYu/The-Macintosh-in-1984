...
class Finder: MacApp {
    ...
    var shouldDragApplication: Bool = false
    ...
    func sizeForWindow() -> CGSize {
        return SystemSettings.finderSize
    }
    ...
    init() {
        ...
        fileMenuList.append(MenuAction(title: "Close", action: {
            if let app = self.desktop!.activeApplications.last {
                self.desktop?.close(app: app)
            }
        },runtimeClosure: {
            if (self.desktop?.activeApplications.count ?? 0) > 0{
                return true
            }
            return false
        }))
        fileMenuList.append(MenuAction(title: "Close All", action: {
            for app in self.desktop!.activeApplications{
                self.desktop?.close(app: app)
            }
        },runtimeClosure: {
            if (self.desktop?.activeApplications.count ?? 0) > 0{
        ...
        fileMenuList.append(MenuAction(title: "Eject  ⌘E", action: nil, subMenus: nil, enabled: false))
        menuActions?.append(MenuAction(title: "File", action: { () in
        }, subMenus: fileMenuList))
        
        var editMenuList = [MenuAction]()
        editMenuList.append(MenuAction(title: "Undo",action: {
            let topApp = self.desktop?.activeApplications.last
        ...
        editMenuList.append(MenuAction(title: "Clear",action: {() in
        ...
        menuActions?.append(MenuAction(title: "Edit", action: { () in
        ...
        menuActions?.append(MenuAction(title: "View", action: { () in
        ...
        menuActions?.append(MenuAction(title: "Special", action: { () in
...
class FinderView: UIView{
    ...
    override func draw(_ rect: CGRect) {
        ...
        var attributes: [NSAttributedStringKey : Any] = [.foregroundColor: UIColor.white,
                                                         .font: SystemSettings.normalSizeFont]
        ...
        attributes = [.foregroundColor: UIColor.white,
                      .font: SystemSettings.notePadFont]
